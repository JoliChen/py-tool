...
class ProjectGroups:
    ...
    def fast_remove_group_by_id(self, group_id, recursive=True):
        group = self.objects[group_id]
        if group is None:
            return False
        to_remove_files = []
        is_ok = self._fast_remove_group_without_file(group, to_remove_files, recursive)
        if not is_ok:
            return False
        removed_files = self.fast_remove_files(to_remove_files, None, None, False)
        return len(removed_files) == len(to_remove_files)

    def _fast_remove_group_without_file(self, group, to_remove_files, recursive=True):
        for sub_item_id in list(group.children):
            sub_item = self.objects[sub_item_id]
            if sub_item is None:
                return False
            if recursive and isinstance(sub_item, PBXGroup):
                self._fast_remove_group_without_file(sub_item, to_remove_files, recursive)
            if isinstance(sub_item, PBXFileReference):
                to_remove_files.append(sub_item.get_id())
        # remove from objects
