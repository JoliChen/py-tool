...
class ProjectFiles:
    ...
    def add_file(self, path, parent=None, tree=TreeType.SOURCE_ROOT, target_name=None, force=True, file_options=FileOptions()):
        ...
        the file type. For instance, framework will be linked, embedded and search paths will be adjusted automatically.
        ...
        # special case for the framework and libraries to update the search paths
    ...
    def add_project(self, path, parent=None, tree=TreeType.GROUP, target_name=None, force=True, file_options=FileOptions()):
        ...
        this project during compilation time. Optional: it can add the products into the different sections: framework
        and bundles.
    ...
    def fast_remove_files(self, file_id_list, target_name=None, build_phase_type=None, check_other_groups=True):
        file_ref_dict = {}
        for file_id in file_id_list:
            file_ref = self.get_file_by_id(file_id)
            if file_ref is not None:
                file_ref_dict[file_ref.get_id()] = file_ref

        for target in self.objects.get_targets(target_name):
            # find target build phase
            build_phase_list = None
            if build_phase_type is None:
                build_phase_list = []
                for build_phase_id in target.buildPhases:
                    build_phase_list.append(self.objects[build_phase_id])
            else:
                build_phase_list = target.get_build_phases(build_phase_type)
            ...
            for build_phase in build_phase_list:
                for build_file_id in list(build_phase.files):
                    ...
                    if build_file.fileRef in file_ref_dict:
        ...
        result = []
        for file_ref_id in file_ref_dict:
            file_ref = file_ref_dict[file_ref_id]
            ...
            del self.objects[file_ref_id]
            result.append(file_ref_id)
        return result
    ...
    def add_folder(self, path, parent=None, excludes=None, recursive=True, create_groups=True, target_name=None,
                   file_options=FileOptions(), tree=TreeType.GROUP, force=True):
        """
        ...
        if (not create_groups) and (os.path.splitext(path)[1] not in ProjectFiles._SPECIAL_FOLDERS):
            return self.add_file(path, parent, tree, target_name, force, file_options)
        ...
        for child in os.listdir(path):
            ...
            if [pattern for pattern in excludes if re.match(pattern, child)]:
                # print 'exclude child : %s' % child
                continue

            full_path = os.path.join(path, child)

            children = []
            if (not create_groups) or os.path.isfile(full_path) or \
                    (os.path.splitext(child)[1] in ProjectFiles._SPECIAL_FOLDERS):
                # check if the file exists already, if not add it
                children = self.add_file(full_path, parent, tree, target_name, force, file_options)
            else:
                # if recursive is true, go deeper, otherwise create the group here.
                if recursive:
                    children = self.add_folder(full_path, parent, excludes, recursive, create_groups,
                                               target_name, file_options, tree, force)
                else:
